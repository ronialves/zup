...
public class ServicoAssinanteRepository extends GenericRepository {
		...
		if (null != serviceId) {
			...
			try {
				...
				logger.log(OSSLogger.INFO,
						String.format("Buscando o serviceId %s na tabela ServicoAssinante...", serviceId));
	...
	@SuppressWarnings("unchecked")
	public ServicoAssinante findByAccessId(ResourceInventoryEntity entity) throws OSSBusinessException {
		...
		List<ServicoAssinante> servicoAssinante = null;

		if (!CpeConstants.ACCESS_DESIGNATOR_PATTERN.matcher(accessId).find()) {
			/*
			 * Se o accessId extraído não tiver o padrão do designador de acesso (-069), a
			 * busca está sendo efetuada pelo designador de produto do cliente, então deve
			 * ser buscado o registro pelo serviceId.
			 */

			sql = "SELECT sa FROM ServicoAssinante sa WHERE sa.serviceId = ?1";

		} else {
			sql = "SELECT sa FROM ServicoAssinante sa WHERE sa.accessId = ?1";
	...
	public List<ServicoAssinante> findByDispositivoFisico(final DispositivoFisico dispositivoFisico) {
		...
		final Query query = factory.createQuery(
				"SELECT df FROM ServicoAssinante sa INNER JOIN RecursoLogico rl ON (sa.id = rl.servicoAssinanteId) WHERE rl.dispositivoFisicoId = ?1");
	...
	@SuppressWarnings("unchecked")
	public List<ServicoAssinante> listByAccessId(ResourceInventoryEntity entity) throws OSSBusinessException {

		logger.log(OSSLogger.INFO, "Extraindo o accessId da entidade...");

		String accessId = EntityExtractor.extractAccessId(entity);

		List<ServicoAssinante> result;
		logger.log(OSSLogger.INFO, "Buscando registro na tabela ServicoAssinante para o AccessId: " + accessId);

		try {
			Query query = factory.createQuery(
					"SELECT sa from ServicoAssinante sa INNER JOIN RecursoLogico rl ON(sa.id = rl.servicoAssinanteId) WHERE sa.accessId = ?1");
