...
public class EquipmentRetrievalControllerTest {
	...
	public void getCabinetByLocationTest() throws OSSBusinessException{
		...
		when(equipmentRetrievalRepository.getEquipment(anyObject())).thenReturn(response);
		ResourceInventoryEntity response = equipmentRetrievalController.retrieval(request);
		
		verify(equipmentRetrievalRepository, times(0)).getEquipment(anyObject());
	...
	public void getEquipmentTreeTest() throws OSSBusinessException {
		...
		verify(equipmentRetrievalRepository).getEquipment(anyObject());
		assertNotNull(response);
	...
	public void retrievalTestOSSBusinessException() throws OSSBusinessException{
		...
		try {	
			when(equipmentRetrievalRepository.getEquipment(anyObject())).thenThrow(exception);
