...
public class InstallControllerTest {
	...
	public void testInstallAccessServicoAssinanteException() throws OSSBusinessException {
		entity.getResourceOrder().getResourceOrderItem().setName(TipoRfs.ACCESS.getRoiName());
		when(servicoAssinanteRepository.findByServiceId(anyObject())).thenReturn(null);
		installController.install(entity);
	}
